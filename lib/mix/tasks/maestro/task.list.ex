defmodule Mix.Tasks.Maestro.Task.List do
  ...
  def run(args) do
    ...
    query = Ash.Query.sort(Maestro.Ops.Task, inserted_at: :desc)
    ...
    query =
      if opts[:type] do
        ...
        Ash.Query.filter(query, task_type == ^type)
      else
        query
      end
    ...
    query =
      if opts[:subtasks] do
        ...
        Ash.Query.filter(query, entity_type == "Task" and entity_id == ^parent_id)
      else
        query
      end
    ...
    tasks = Ash.read!(query, authorize?: false)
