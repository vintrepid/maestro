defmodule Maestro.Ops.Rules.LintExtractor do
  ...
  @extractors [
    {~r/deprecated.*let=\{|let=\{.*deprecated/i,
     %{
       lint_pattern: "let=\\{[a-z]",
       lint_file_types: ["heex"],
    ...
    {~r/never.*`?(form_for|Phoenix\.HTML\.form_for)|deprecated.*(form_for|Phoenix\.HTML\.form_for)/i,
     %{
       lint_pattern: "Phoenix\\.HTML\\.form_for|Phoenix\\.HTML\\.inputs_for",
       lint_file_types: ["ex", "heex"],
       lint_message:
         "Deprecated Phoenix.HTML.form_for/inputs_for. Use Phoenix.Component.form/1 and inputs_for/1."
     }},
    {~r/never.*`?<% Enum\.each|Enum\.each.*template|never.*Enum\.each/i,
     %{
       lint_pattern: "<%.*Enum\\.each",
       lint_file_types: ["heex"],
       lint_message:
         "Don't use Enum.each in templates. Use <%= for item <- @collection do %> instead."
     }},
    ...
    {~r/never.*HTML comment|HTML comment.*never|<%!--.*instead of.*<!--|NEVER use HTML comments/i,
     %{
       ...
       lint_file_types: ["heex"],
       lint_message:
         "HTML comment <!-- --> will be sent to client. Use HEEx comment <%!-- --%> instead."
     }},
    {~r/else.?if.*never|never.*else.?if|does not support.*else.?if|NEVER use else if/i,
     %{
       lint_pattern: "else\\s+if\\b",
       lint_file_types: ["heex", "ex"],
    ...
    {~r/never.*phx-update=.?(append|prepend)|deprecated.*phx-update/i,
     %{
       lint_pattern: "phx-update=[\"'](append|prepend)[\"']",
       lint_file_types: ["heex"],
    ...
    {~r/never.*`?(live_redirect|live_patch)\b|deprecated.*(live_redirect|live_patch)/i,
     %{
       lint_pattern: "\\blive_redirect\\b|\\blive_patch\\b",
       lint_file_types: ["ex", "heex"],
       lint_message:
         "Deprecated live_redirect/live_patch. Use push_navigate/push_patch or <.link navigate={}/patch={}>."
     }},
    {~r/never.*Phoenix\.View|Phoenix\.View.*deprecated|Phoenix\.View.*no longer|don't use.*Phoenix\.View/i,
     %{
       lint_pattern: "Phoenix\\.View",
       lint_file_types: ["ex"],
    ...
    {~r/don't use `?String\.to_atom|String\.to_atom.*unsafe|never.*String\.to_atom/i,
     %{
       lint_pattern: "String\\.to_atom\\(",
       lint_file_types: ["ex"],
       lint_message:
         "String.to_atom/1 is unsafe on user input (atom table is not GC'd). Use String.to_existing_atom/1."
     }},
    {~r/never.*raw Ecto|Ash actions instead.*from\(\)|never.*from\(\).*query/i,
     %{
       lint_pattern: "\\bfrom\\s+\\w+\\s+in\\b",
       lint_file_types: ["ex"],
    ...
    {~r/never.*<script.*HEEx|embedded.*<script|never.*<script.*template/i,
     %{
       ...
       lint_file_types: ["heex"],
       lint_message:
         "Embedded <script> tag in HEEx. Write hooks in assets/js/ and integrate with app.js instead."
     }},
    {~r/never.*nested.*defmodule|defmodule.*same file|never.*nest modules/i,
     %{
       lint_pattern: "^\\s+defmodule\\s",
       lint_file_types: ["ex"],
    ...
    {~r/Process\.sleep.*test|test.*Process\.sleep|avoid.*Process\.sleep/i,
     %{
       lint_pattern: "Process\\.sleep",
       lint_file_types: ["ex"],
       lint_message:
         "Avoid Process.sleep in tests. Use Process.monitor and assert on DOWN message instead.",
       lint_only_paths: ["test/"]
     }},
    {~r/phx-hook.*require.*id|phx-hook.*unique.*id/i,
     %{
       lint_pattern: "phx-hook=",
       lint_file_types: ["heex"],
       lint_message:
         "phx-hook requires a unique DOM id on the same element. Verify id={...} is present."
     }},
    {~r/LiveComponent.*avoid|avoid.*LiveComponent/i,
     %{
       lint_pattern: "live_component\\b|LiveComponent\\b",
       lint_file_types: ["ex", "heex"],
       lint_message:
         "LiveComponent detected. Avoid LiveComponents unless you have a strong, specific need."
     }},
    ...
    {~r/predicate.*should not start with `is_|is_.*should end.*\?/i,
     %{
       lint_pattern: "def is_\\w+[^?]\\(",
       lint_file_types: ["ex"],
       lint_message:
         "Predicate functions should not start with is_ and should end with ?. Use foo? instead of is_foo."
     }}
  ]
