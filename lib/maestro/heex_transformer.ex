defmodule Maestro.HEExTransformer do
  ...
  @spec transform_section_cards(any()) :: term()
  def transform_section_cards(heex_string) do
          ...
          case class_attr do
            {"class", class} when is_binary(class) ->
              if String.contains?(class, "card bg-base-100 shadow-xl") do
                extra = String.trim(String.replace(class, "card bg-base-100 shadow-xl", ""))
    ...
    html
    |> String.replace(
      ~r/<PHOENIX_SECTION_CARD data-component-class="([^"]+)">/,
      "<.section_card class=\"\\1\">"
    )
  ...
  @spec transform_stats_grids(any()) :: term()
  def transform_stats_grids(heex_string) do
          ...
          case class_attr do
            {"class", class} when is_binary(class) ->
              if String.contains?(class, "stats stats-vertical lg:stats-horizontal") do
                ...
                extra = String.trim(String.replace(class, base_pattern, ""))
    ...
    html
    |> String.replace(
      ~r/<PHOENIX_STATS_GRID data-component-class="([^"]+)">/,
      "<.stats_grid class=\"\\1\">"
    )
