defmodule MaestroWeb.AdminLive.TailwindAnalysisLive do
  ...
  def fields do
    [
      class_name: %{label: "Class", sortable: true, searchable: true, renderer: &render_class_name/1},
      category: %{label: "Category", sortable: true, searchable: true, renderer: &render_category/1},
                ...
                <table class="table table-sm">
                  ...
                  <tbody>
                    <%= for stat <- Enum.take(@summary_stats, 20) do %>
                      <tr class="cursor-pointer hover" phx-click="select_class" phx-value-class={stat.class_name}>
                        ...
                        <td class="font-semibold text-right">{stat.total_occurrences}</td>
                ...
                <%= for stat <- @category_stats do %>
                  <div class="flex justify-between items-center p-2 rounded hover:bg-base-200">
                    ...
                    <div class="text-right">
                      ...
                      <div class="text-xs text-base-content/60">{stat.unique_classes} unique</div>
                    </div>
                  </div>
                <% end %>
              </div>
  ...
  def handle_event("run_analysis", %{"description" => description}, socket) do
    ...
    Task.start(fn ->
      System.cmd("mix", ["analyze_tailwind", "--load-db"] ++ 
