defmodule MaestroWeb.ConceptsController do
  @moduledoc """
  Controller for Concepts routes.
  """
  use MaestroWeb, :controller

  @spec index(Plug.Conn.t(), map()) :: term()
  def index(conn, _params) do
    svg_path =
      Path.join([Application.app_dir(:maestro), "priv", "static", "images", "concept_dag.svg"])
  ...
  @spec directory(Plug.Conn.t(), map()) :: term()
  def directory(conn, %{"dir" => dir}) do
    ...
    if File.dir?(dir_path) do
      svg_path =
        Path.join([
          Application.app_dir(:maestro),
          "priv",
          "static",
          "images",
          "concept_dag_#{dir}.svg"
        ])
  ...
  @spec file(Plug.Conn.t(), map()) :: term()
  ...
  @spec bundle_graph(Plug.Conn.t(), map()) :: term()
