...
**ALWAYS** **Always** Regex lint patterns cannot fix code — they can only detect. Every regex lint rule should be converted to a MaestroTool.Lint.Check module with AST-aware detection and fix/2. Regex patterns also produce false positives because they lack context (e.g., matching `:for=` comprehensions as form attributes). AST checks are correct by construction.
**ALWAYS** Never store learnings, feedback, or corrections in Claude memory files. Create Rules in Maestro DB instead — that's the shared learning system across all agents and all projects. Memory files are local to one agent; Rules propagate to every agent via CLAUDE.md generation.
...
**ALWAYS** Never ask the user for permission to read files, run tools, or execute basic operations. If a tool is available, use it. The user has already granted access by installing and running the agent. Stopping to ask permission on routine operations wastes the user's time and breaks flow.
...
**ALWAYS** **Never** Every MaestroTool.Lint.Check module follows this pattern:
...
- **Prefer** flat module structure. The top level of a context should reveal the domain — not bury it under nested directories. If you need to descend 3+ directories to find the domain logic, the structure is taxonomy masquerading as architecture. Group by domain concept, not by technical layer.
...
- **Always** [maestro_tool shared package](project_maestro_tool.md) — shared dep for all projects; agent tools belong there, not in Maestro
...
- **Always** Rules propagate to ALL agents in ALL projects via CLAUDE.md generation. Memory files are local and useless.
- **Always** Rules DB at /rules is the shared learning system. Check rules before starting work.
- **Always** [feedback_fix_tool_first.md](feedback_fix_tool_first.md) — Update the tool + rule first, then run the tool to apply changes
- - Class lists: always use `[...]` syntax with conditional classes: `class={["base", @flag && "extra"]}`.
- **Always** [project_session_scope.md](project_session_scope.md) — Design site session scope (visit→user→season) for Ash actor propagation
...
- **Always** say "You're right" or similar validating phrases. When corrected, acknowledge the correction by restating the correct approach and immediately applying it. Don't waste the user's time with empty agreement — show you understood by doing.
...
- **Always** Hypothesis-forming: list the hypotheses briefly and which I'm
...
- **Always** Approach choices (refactor path, rename strategy, test harness
  shape): 2–3 lines of trade-off before committing to the path.
- **Always** Long waits: a line saying what I'm waiting for, then quiet until
...
- **Always** **Re-streaming on assign change**: When updating an assign that affects streamed items, re-stream the affected items alongside the assign change.
...
- **Always** remember: `<.flash_group>` lives in `Layouts` module only (Phoenix 1.8+) — never call it elsewhere.
...
- **Always** Implement delightful UI details: hover effects, loading states, smooth transitions. Internal apps optimize for information density; micro-interactions should aid usability, not decoration.
...
**ALWAYS** **Never** use bracket access on Elixir lists — `list[0]` returns `nil` silently (lists don't implement Access). Use `Enum.at(list, 0)`, `hd(list)`, or pattern matching `[first | _rest] = list`. — otherwise the result will be incorrect or break downstream behavior
...
**ALWAYS** **Always** `%{}` matches ANY map, not just empty maps. Use `map_size(map) == 0` guard to check for truly empty maps. — because skipping this step causes inconsistent or broken results
- **Always** `mix deps.clean --all` is almost never needed. Avoid it unless you have a specific reason. For test debugging, use `mix test path/to/test.exs` for a specific file or `mix test --failed` for previously failed tests.
...
- **Prefer** process-first naming: name the action, parameterize the participants. Processors end in `-er` (validator, parser, builder). Contracts/interfaces end in `-able` (validatable, parseable, buildable). The process owns the verb; the nouns are participants. This is Interface Segregation made manifest in vocabulary.
...
- **Always** remember: `<.icon name="hero-x-mark" class="w-5 h-5"/>` for icons — never use Heroicons modules.
...
- **Always** Literal curly braces in templates require `phx-no-curly-interpolation` on the parent tag.
...
- **Always** **Naming**: `AppWeb.WeatherLive` with `Live` suffix. Router scope provides the alias — `live "/weather", WeatherLive`.
